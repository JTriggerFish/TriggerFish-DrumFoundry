...
bool AnalysisView::mouseWheel(const visage::MouseEvent &e) {
  if (e.isAltDown()) {
    ...
    const double fraction =
        (At(e.position.x, e.position.y).time - pan) / span;
    const double next =
...
void AnalysisView::mouseDown(const visage::MouseEvent &e) {
  if (!e.isLeftButton() || e.position.y < PlotTop ||
      e.position.y >= height() - 29 || e.position.x < 42 ||
      e.position.x >= width() - 12)
    return;
...
void AnalysisView::mouseDrag(const visage::MouseEvent &e) {
  if (!dragging_)
    return;
  if (divider_) {
    split = Mode() == Comparison::Stacked
                ? (e.position.y - PlotTop) /
                      std::max(1.f, height() - PlotTop - 29)
