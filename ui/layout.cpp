...
void Workbench::resized() {
  ...
  preset_.setBounds(16, 14, 150 * scale, 30);
  settings_.setBounds(width() - 16 - 100 * scale, 14, 100 * scale, 30);
  master_.setBounds(settings_.x() - 198, 6, 184, 44);
  limiter_.setBounds(master_.x() - 14 - 138 * scale, 14, 138 * scale, 30);
  const float col = (left - 48) / 2;
  routingToggle_.setBounds(16, 70, left - 32, 28);
  routing_.setBounds(16, 102, left - 32, 150);
  ...
  columnSplit_.setBounds(left - 4, 70, 10, height() - 195);
  right_.setBounds(left + 16, 70, width() - left - 32, height() - 195);
  LayoutRight();
  ...
  excitation_.setBounds(16, controlsTop + controlsOffset,
                        single ? left - 32 : col,
                        height() - controlsTop - controlsOffset - 125);
  resonance_.setBounds(single ? 16 : 32 + col, controlsTop + controlsOffset,
                       single ? left - 32 : col,
                       height() - controlsTop - controlsOffset - 125);
  history_.setBounds(16, height() - 114, width() - 32, 34);
...
void Workbench::draw(visage::Canvas &c) {
  ...
  Label(c, ElideText(FrameFont(*this), title, titleWidth), titleX, 14,
        titleWidth, 30, colours::Heading);
  c.setColor(colours::Grid);
  c.fill(0, 58, width(), 1);
  c.fill(left, 58, 1, height() - 183);
  if (!SingleColumn()) {
  ...
  Label(c, meter, 16, height() - 79, width() - 32, 20,
        reduction_ > .1 ? colours::Warning : colours::Muted);
  Label(c, status_, 16, height() - 55, width() - 32, 24, colours::Muted);
  Label(c, error_, 16, height() - 29, width() - 32, 24, colours::Error);
}
} // namespace drumfoundry::ui
