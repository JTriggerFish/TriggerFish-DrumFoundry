...
def test_standalone_bass_loss_and_metadata():
    rate, _, reference, _ = signals()
    loss = RidgeBalanceLoss(reference, reference, rate, low_resolution=True)
    assert np.linalg.norm(loss.residual(reference)) == 0
    assert loss.diagnostics(reference)["bass_ridge_error"] == 0
    assert loss.specification["fft"] == 8192
    assert loss.specification["hz"] == [20, 250]
    assert loss.diagnostics(reference * 0.5)["combined_error"] > 0
