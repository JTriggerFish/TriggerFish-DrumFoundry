...
void TestStatisticalCloudIsDeterministicAndNormalized() {
  ...
  for (std::size_t mode = 0; mode < Count; ++mode) {
    ...
    if (mode > 0)
      ordered =
          ordered && first[mode].frequencyHz > first[mode - 1].frequencyHz;
    squaredGain += first[mode].outputGain * first[mode].outputGain;
    changed = changed || first[mode].frequencyHz != different[mode].frequencyHz;
    Check(first[mode].frequencyHz == repeated[mode].frequencyHz &&
              ...
              first[mode].inputPhaseRadians == repeated[mode].inputPhaseRadians,
...
void TestDenseCloudGainEnvelopeShapesBroadBands() {
  ...
  for (std::size_t point = 0; point < shaped.gainEnvelopeDb.size(); ++point)
    shaped.gainEnvelopeDb[point] =
        -12.f + 24.f * static_cast<float>(point) /
...
void TestDenseCloudDecayEnvelopeShapesMiddleBand() {
  ...
  CheckNear(shapedModes.front().decaySeconds, flatModes.front().decaySeconds,
            .02, "modal-cloud decay envelope preserves the low endpoint");
  CheckNear(shapedModes.back().decaySeconds, flatModes.back().decaySeconds, .02,
...
void TestTurbulentResidualStoresAndPassivelyLosesEnergy() {
  ...
  Check(
      charged > 0.f && residual.StoredEnergy() < charged,
