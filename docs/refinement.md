...
Modal series are generated by `drumfoundry.editing.generate_series`, a binding
to the editor's C++ implementation. Shared-series fitting uses that same
protected-core stretch, applied as a relative deformation to existing handles;
negative search coordinates apply its reciprocal. There is no separate Python
stretch law. This changes new search candidates, not stored preset parameters.
Saved-frequency deformation and range validation also run in C++ through
`drumfoundry.editing.transform_series`; an infeasible candidate is rejected before
rendering, never clamped into duplicate edge modes.
The optimizer receives an infinite constraint cost, not a recorded loss value.
`LayerFit(workers=1)` and `LayerFit(workers=N)` share evaluation, seeds, scoring
and trial recording; only scheduling and native voice ownership differ.
Failed parallel batches cancel pending work and drain running tasks before
returning an error, so a retry cannot overlap access to the same native voice.
