...
class RidgeBalanceLoss:
    ...
    def __init__(self, reference, baseline, rate, low_resolution=False):
        ...
        self.specification = dict(
            ...
            fft=self.config.window_samples,
            hop=256,
            regions=self.regions,
            hz=[20, 250] if low_resolution else [100, 4000],
            frequency_smoothing_bins=1 if low_resolution else 3,
    ...
    def residual(self, samples, regions=range(5)):
        ...
        if self.low is not None:
            terms.append(self.low.ridge_residual(samples, regions))
        return np.concatenate(terms) / np.sqrt(len(terms))
    ...
    def diagnostics(self, samples):
        ...
        bass = self.low if self.low is not None else self
        result["bass_ridge_error"] = float(
            np.linalg.norm(bass.ridge_residual(samples, range(5)))
        )
