...
/***
 * References : Digital signal processing schemes for efficient interpolation
 * and decimation, R.A Valenzuela, A.G Constantinides, IEE 1983
 */
...
class PolyphaseIIR_X2Resampler
    : public Resampler<PolyphaseIIR_X2Resampler<N, M>, 2> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  PolyphaseIIR_X2Resampler(const Eigen::Array<double, N, 1> &coeffsDirect,
                           const Eigen::Array<double, M, 1> &coeffsDelayed)
      : _coeffsDirect(coeffsDirect), _coeffsDelayed(coeffsDelayed) {
...
public:
  explicit X4Resampler(
      std::function<std::unique_ptr<X2Type>()> resamplerCreator)
      : _stage1{resamplerCreator()}, _stage2(resamplerCreator()) {}
