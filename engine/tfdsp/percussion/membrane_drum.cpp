...
ContactExciterParameters ContactForHit(ContactExciterParameters result,
  ...
  const float mallet =
      std::clamp(1.f - 2.f * std::abs(implement - .5f), 0.f, 1.f);
  const float stick = std::clamp(2.f * implement - 1.f, 0.f, 1.f);

  result.pulseDurationSeconds *=
      .45f * stick + 1.7f * mallet + (2.f + 3.f * spread) * brush;
  ...
  result.microContacts.brightness =
      Unit(result.microContacts.brightness + .35f * (hardness - .5f));
  result.microContacts.amplitude *=
      .18f * stick + .42f * mallet + (1.25f + .5f * spread) * brush;
...
MembraneDrum::EventVoice::Sample MembraneDrum::EventVoice::Process() noexcept {
  ...
  last = result;
  const float peak =
      std::max({std::abs(result.contactDirect), std::abs(result.contactBody),
                std::abs(result.fm)});
...
MembraneDrum::EventVoice &MembraneDrum::NextVoice() noexcept {
  const auto inactive =
      std::find_if(voices_.begin(), voices_.end(),
